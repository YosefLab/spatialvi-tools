...
from spatialvi.model.scviva.differential_expression import DifferentialExpressionResults
...
def split_ref_query_adata(adata: AnnData):
    ...
    adata.obs["hemisphere"] = ["right" if x > 0 else "left" for x in adata.obsm["coordinates"][:, 0]]
...
def test_scviva_scarches_less_features(split_ref_query_adata):
    ...
    query_adata = query_adata[:, np.random.permutation(query_adata.var_names)[: query_adata.shape[1] - 5]].copy()

    query_adata.obs["labels"] = query_adata.obs["labels"].astype(str).replace("label_2", "label_1").astype("category")
    # Query adata
...
def test_scviva_scarches_same_features(split_ref_query_adata):
    ...
    assert ref_labels == query_labels, f"Label sets do not match:\nRef: {ref_labels}\nQuery: {query_labels}"
    ...
    query_adata = query_adata[:, np.random.permutation(query_adata.var_names)[: query_adata.shape[1]]].copy()
