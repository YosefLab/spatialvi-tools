...
def test_diagvi_from_spatialdata_not_supported():
    """DIAGVI is multi-modal; the inherited single-adata from_spatialdata must not
    silently misbehave.
    """
    from scviva.external.diagvi import DIAGVI

    with pytest.raises(NotImplementedError, match="setup_spatialdata"):
        DIAGVI.from_spatialdata(sdata=object())


def test_diagvi_plot_spatial_embedding_requires_explicit_adata(adata_seq, adata_spatial):
    model = make_model(adata_seq, adata_spatial)
