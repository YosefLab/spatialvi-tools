"""scVIVA module implementation.

This module contains the neural network components for scVIVA,
a model for learning niche-aware representations in spatial transcriptomics.
"""
...
class nicheVAE(VAE):
    ...
    Extends the base VAE to model cellular microenvironments by learning
    niche-aware representations that capture both cell-intrinsic and
    ...
    n_output_niche
        Number of output features for niche state.
    n_batch
    ...
    log_variational
        If ``True``, use log1p on input data before encoding.
    gene_likelihood
        Distribution to use for reconstruction.
    latent_distribution
        Distribution to use for the latent space.
    niche_likelihood
        Distribution to use for the niche state.
    cell_rec_weight
    ...
    prior_mixture
        If ``True``, use a mixture of Gaussians for the latent space.
    prior_mixture_k
        Number of components in the Gaussian mixture.
    semisupervised
        If ``True``, use a classifier to predict cell type labels.
    linear_classifier
        If ``True``, use a linear classifier.
    inpute_covariates_niche_decoder
        If ``True``, covariates are concatenated to the input of the niche decoder.
    encode_covariates
        If ``True``, covariates are concatenated to gene expression before encoding.
    deeply_inject_covariates
        If ``True`` and ``n_layers > 1``, covariates are injected into hidden layers.
    ...
    use_size_factor_key
        If ``True``, use the size factor from obs.
    use_observed_lib_size
        If ``True``, use observed library size for RNA.
    library_log_means
        Means of the log library sizes for prior.
    library_log_vars
        Variances of the log library sizes for prior.
    batch_embedding_kwargs
        Keyword arguments for batch embedding.
    extra_decoder_kwargs
        Additional keyword arguments for decoder.
    extra_encoder_kwargs
        Additional keyword arguments for encoder.
    **vae_kwargs
        Additional keyword arguments for base VAE.
    """
    ...
    def loss(
        ...
        n_samples_mixture
            Number of samples for mixture prior.
        ...
        if self.prior_mixture is True:
            z = inference_outputs[MODULE_KEYS.QZ_KEY].rsample(sample_shape=(n_samples_mixture,))
            kl_divergence_z = (
