...
class MRDeconv(EmbeddingModuleMixin, BaseModuleClass):
    ...
    def _get_inference_input(self, tensors):
        ...
        if self.augmentation and self.training:
            with torch.no_grad():
                prior_sampled = self.qz_prior.sample([n_samples, m]).reshape(n_samples, -1, self.n_latent)
                ...
                batch_rep_input = batch_rep.repeat_interleave(self.n_labels, dim=0).repeat(n_samples, 1, 1)
                ...
                ratios_ct_augmentation = (
                    torch.distributions.Dirichlet(torch.zeros(self.n_labels) + 0.03).sample([n_samples, m]).to(x.device)
                )
                ratios_ct_augmentation = ratios_ct_augmentation.reshape(n_samples, m, self.n_labels)
                # sum over celltypes
                ...
                augmented_counts = NegativeBinomial(mu=augmentation_rate, theta=px_r).sample()  # (samples, m, n_genes)
                x_augmented = (1 - ratio_augmentation) * x.unsqueeze(0) + ratio_augmentation * augmented_counts
                if x_smoothed is not None:
    ...
    def generative(self, z, ind_x, library, batch_index):
        ...
        if self.amortization in ["both", "latent"]:
            if self.prior_mode == "mog":
                ...
                proportion_modes_logits = (
                    torch.transpose(gamma_[:, :, -self.n_states_per_label * self.n_labels :], -1, -2)
        ...
        if self.amortization in ["both", "proportion"]:
            ...
            v_ind = self.V[:, ind_x].T.unsqueeze(0).repeat(n_samples, 1, 1)  # n_samples, m, labels + 1
        ...
        for mode in range(gamma_ind.shape[-4]):
            ...
            gamma_ind_ = torch.transpose(gamma_ind[:, mode, ...], -1, -3)  # n_samples, m, n_labels, n_latent
            gamma_reshape_ = gamma_ind_.reshape((n_samples, -1, self.n_latent))  # n_samples, m * n_labels, n_latent
        ...
        px_scale = torch.sum((v_ind.unsqueeze(-1) + eps_v) * r_hat, dim=-2)  # n_samples, m, n_genes
    ...
    def loss(
        ...
        reconst_loss = -NegativeBinomial(mu=px_rate, theta=px_r).log_prob(x_augmented).sum(-1).mean(0)
        ...
        if self.mean_vprior is None:
            ...
            normal_dists = Independent(Normal(mean_vprior, var_vprior), reinterpreted_batch_ndims=1)
            ...
            log_likelihood_prior = torch.mul(pre_lse, proportion_modes + 1e-6).sum(-3)  # n_samples, minibatch, n_labels
            neg_log_likelihood_prior = -torch.mul(log_likelihood_prior, v[:, :, : -self.add_celltypes] + 1e-3).sum(
                -1
