...
from spatialvi.module import MRDeconv
from spatialvi.train._config import merge_kwargs
    ...
    **module_kwargs
    ...
    def from_rna_model(
        ...
        **model_kwargs
        ...
        decoder_state_dict = OrderedDict(
            (i[8:], load_state_dict[i]) for i in load_state_dict.keys() if i.split(".")[0] == "decoder"
        )
        px_decoder_state_dict = OrderedDict(
            (i[11:], load_state_dict[i]) for i in load_state_dict.keys() if i.split(".")[0] == "px_decoder"
        )
        ...
        if vamp_prior_p is None:
            ...
            mean_vprior, var_vprior, mp_vprior = sc_model.get_vamp_prior(sc_model.adata, p=vamp_prior_p).values()
    ...
    def get_fine_celltypes(
        ...
        if sc_model.n_fine_labels is None:
            raise RuntimeError(
                "Single cell model does not contain fine labels. Please train the single-cell model with fine labels."
            )
        ...
        for tensors in stdl:
            ...
            v_local = (
                generative_outputs["v"][0, ..., : -self.module.add_celltypes].flatten().repeat_interleave(n_modes)
            )  # m*p*c
            label = (
            ...
            predicted_fine_celltype_local = (
                ...
                * torch.nn.functional.softmax(sc_model.module.classify(gamma_local_, label), dim=-1)
            )
    ...
    def get_latent_representation(
        ...
        """
        assert self.module.n_latent_amortization is not None, "Model has no latent representation for amortized values."
        ...
        return (
            (torch.cat(latent_qzm).numpy(), torch.cat(latent_qzv).numpy()) if return_dist else torch.cat(latent).numpy()
        )
