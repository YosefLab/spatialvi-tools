...
from spatialvi.model.scviva import SCVIVA_REGISTRY_KEYS
    ...
    **kwargs,
    ...
    for g1 in track(
        ...
        if isinstance(fdr, list):
            assert len(fdr) == len(comparisons), f"Mismatch: len(fdr)={len(fdr)}, expected={len(comparisons)}"
        else:
            fdr = [fdr] * len(comparisons)  # Convert to list of same value

        if isinstance(delta, list):
            assert len(delta) == len(comparisons), f"Mismatch: len(delta)={len(delta)}, expected={len(comparisons)}"
        else:
    ...
    fdr_g1_n1 = DE_results["group1_neighbors1"][f"is_de_fdr_{DE_group_fdr['group1_neighbors1']}"].copy()
