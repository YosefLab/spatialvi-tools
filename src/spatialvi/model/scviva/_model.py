    ...
    def predict_neighborhood(
        ...
        for tensors in scdl:
            ...
            ct_prediction.append(
                (predicted_ct_prob.concentration / predicted_ct_prob.concentration.sum(dim=1).unsqueeze(1))
                .detach()
                .cpu()
            )
...
def get_niche_indexes(
    ...
    print(f"[bold cyan]Saved {niche_indexes_key} and {niche_distances_key} in adata.obsm[/bold cyan]")
...
def _pad_and_sort_query_anndata(
    ...
    if len(extra_in_query) > 0:
        raise ValueError(f"Label(s) observed in query but not in reference: {sorted(extra_in_query.tolist())}")
    ...
    if len(missing_in_query) > 0:
        ...
        for ct in missing_in_query:
            ...
            zeros = np.zeros((adata.n_obs, 1, adata.obsm[expression_embedding_niche_key].shape[-1]))
