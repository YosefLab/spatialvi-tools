...
class ResolVIPredictiveMixin:
    ...
    def get_normalized_expression_importance(
        ...
        for tensors in scdl:
            ...
            log_weights = (
                torch.distributions.Poisson(samples[0, ...] + 1e-3).log_prob(kwargs["x"].to(samples.device)).sum(-1)
            )
    ...
    def get_neighbor_abundance(
        ...
        **kwargs,
        ...
        if neighbor_key is None:
            neighbor_key = self.adata_manager.registry["field_registries"]["index_neighbor"]["data_registry"][
                "attr_key"
            ]
        ...
        dl = self._make_data_loader(adata=adata, indices=indices_, shuffle=False, batch_size=batch_size)
