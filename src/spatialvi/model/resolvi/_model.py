...
class RESOLVI(PyroSviTrainMixin, PyroSampleMixin, ResolVIPredictiveMixin, BaseModelClass, ArchesMixin):
    """
    ...
    **model_kwargs
        ...
        plan_kwargs.update(
            {
                ...
                "loss_fn": Trace_ELBO(num_particles=5, vectorize_particles=True, retain_graph=True),
            }
        )
    ...
    def setup_anndata(
        ...
        **kwargs,
        ...
        if labels_key is None:
            ...
            label_field = LabelsWithUnlabeledObsField(REGISTRY_KEYS.LABELS_KEY, labels_key, unlabeled_category)
    ...
    @staticmethod
    def _prepare_data(adata, n_neighbors=10, spatial_rep="X_spatial", batch_key=None, slice_key=None, **kwargs):
        ...
        try:
            ...
            raise ImportError("Please install scanpy and scikit-learn -- `pip install scanpy`") from err
        ...
        for index in indices:
            ...
            try:
                ...
                rapids_singlecell.pp.neighbors(sub_data, n_neighbors=n_neighbors + 5, use_rep=spatial_rep)
            except ImportError:
    ...
    def compute_dataset_dependent_priors(self, n_small_genes=None):
        ...
        smallest_means = x[:, np.array(x.sum(0)).flatten().argsort()[:n_small_genes]].mean(1) / np.array(x.mean(1))
