...
def setup_anndata(
    ...
    for ct in cell_types:
        ...
        obs = {barcode_key: obs_names.values, cell_type_key: [ct] * len(obs_names)}
...
def modify_uns_hotspot(adata):
    ...
    if "lc_zs" in adata.uns.keys():
        genes = [
            " - ".join(gene) if isinstance(gene, tuple) else gene
            for gene in adata.uns["lc_zs"].columns
        ]
...
def modify_uns_harreman(adata):
    ...
    if "LR_database" in adata.uns.keys():
        adata.uns["LR_database"].columns = [
            col.replace(".", "_") for col in adata.uns["LR_database"].columns
        ]
        adata.uns["LR_database"]["ligand_transmembrane"] = adata.uns["LR_database"][
            "ligand_transmembrane"
        ].astype(str)
        adata.uns["LR_database"]["receptor_transmembrane"] = adata.uns["LR_database"][
            "receptor_transmembrane"
        ].astype(str)
    ...
    if "gene_pairs_per_ct_pair" in adata.uns.keys():
        ...
        adata.uns["gene_pairs_per_ct_pair"] = {
            " - ".join(key): value for key, value in adata.uns["gene_pairs_per_ct_pair"].items()
        }

    if "gene_pairs_per_ct_pair_ind" in adata.uns.keys():
        adata.uns["gene_pairs_per_ct_pair_ind"] = {
            " - ".join(key): value
            for key, value in adata.uns["gene_pairs_per_ct_pair_ind"].items()
        }

    if "gene_pairs_per_ct_pair_sig_ind" in adata.uns.keys():
        adata.uns["gene_pairs_per_ct_pair_sig_ind"] = {
            " - ".join(key): value
            for key, value in adata.uns["gene_pairs_per_ct_pair_sig_ind"].items()
        }
    ...
    if "gene_pairs_ind_per_ct_pair" in adata.uns.keys():
        ...
        adata.uns["gene_pairs_ind_per_ct_pair"] = {
            " - ".join(key): value
            for key, value in adata.uns["gene_pairs_ind_per_ct_pair"].items()
        }
    ...
    if "gene_pairs_ind_per_ct_pair_sig" in adata.uns.keys():
        ...
        adata.uns["gene_pairs_ind_per_ct_pair_sig"] = {
            " - ".join(key): value
            for key, value in adata.uns["gene_pairs_ind_per_ct_pair_sig"].items()
        }
...
def recover_uns_hotspot(adata):
    ...
    if "lc_zs" in adata.uns:
        adata.uns["lc_zs"].index = [
            tuple(g.split(" - ")) if " - " in g else g for g in adata.uns["lc_zs"].index
        ]
        adata.uns["lc_zs"].columns = [
            tuple(g.split(" - ")) if " - " in g else g for g in adata.uns["lc_zs"].columns
        ]
...
def recover_uns_harreman(adata):
    ...
    if "gene_pairs" in adata.uns:
        ...
        gene_pairs_tmp = [
            (x, list(y.split(" - ")) if " - " in y else y) for x, y in gene_pairs_tmp
        ]
        adata.uns["gene_pairs"] = [
            (list(x.split(" - ")) if " - " in x else x, y) for x, y in gene_pairs_tmp
        ]

    if "gene_pairs_sig" in adata.uns:
        gene_pairs_sig_tmp = [tuple(gp.split("_")) for gp in adata.uns["gene_pairs_sig"]]
        gene_pairs_sig_tmp = [
            (x, list(y.split(" - ")) if " - " in y else y) for x, y in gene_pairs_sig_tmp
        ]
        adata.uns["gene_pairs_sig"] = [
            (list(x.split(" - ")) if " - " in x else x, y) for x, y in gene_pairs_sig_tmp
        ]
    ...
    if "gene_pairs_per_ct_pair" in adata.uns:
        adata.uns["gene_pairs_per_ct_pair"] = {
            tuple(k.split(" - ")): [
                (recover_tuple_or_list(pair[0]), recover_tuple_or_list(pair[1])) for pair in v
            ]
    ...
    if "gene_pairs_per_ct_pair_ind" in adata.uns:
        adata.uns["gene_pairs_per_ct_pair_ind"] = {
            tuple(k.split(" - ")): v for k, v in adata.uns["gene_pairs_per_ct_pair_ind"].items()
        }
