...
# SpatialPredictiveMixin Generalization & DE Coverage

**Date:** 2026-06-07
**Status:** Implemented
**Full spec:** `docs/superpowers/specs/2026-06-07-spatial-predictive-mixin-and-de-design.md`

---

## Summary

Two improvements added after the AMICI/Starfysh phase:

1. **`ResolVIPredictiveMixin` generalized → `SpatialPredictiveMixin`**: The mixin is renamed
   and applied to both SCVIVA and ResolVI. SCVIVA gains `get_normalized_expression_importance`
   (PyTorch default: delegates to `get_normalized_expression`) and `get_neighbor_abundance`
   (returns precomputed `niche_composition` from obsm). ResolVI retains its Pyro overrides for
   all three methods in its class body. A backward-compat alias `ResolVIPredictiveMixin` is kept.

2. **DE coverage**: `differential_expression` via `RNASeqMixin` was the target for
   GIMVI and RNAStereoscope, but both were found incompatible at runtime:
   - GIMVI: `JVAE.generative()` uses a dual-dataset interface incompatible with
     `RNASeqMixin.get_normalized_expression`.
   - RNAStereoscope: `RNADeconv.inference()` does not accept `n_samples`, making
     `RNASeqMixin.get_normalized_expression` unusable.
   Both remain without `differential_expression`. Only SCVIVA and ResolVI support DE.

   `RNAStereoscope.setup_anndata` gained an optional `batch_key` parameter to enable
   future DE work without changing the model's core class hierarchy.
...
| `src/scviva/model/base/_resolvi_predictive.py` | Replaced with alias shim |
| `src/scviva/model/base/__init__.py` | Exports `SpatialPredictiveMixin` + alias |
| `src/scviva/model/_scviva.py` | Added `SpatialPredictiveMixin` to MRO; `get_neighbor_abundance` override |
| `src/scviva/model/_resolvi.py` | MRO: `SpatialPredictiveMixin`; Pyro overrides in class body |
| `src/scviva/external/stereoscope/_model.py` | `batch_key` param added to `setup_anndata` |

## New Tests

| Test | Model | What it checks |
|------|-------|---------------|
| `tests/base/test_spatial_predictive.py` | ResolVI, SCVIVA | mixin-level: neighbor abundance shape + NE importance shape |
| `test_scviva_normalized_expression` | SCVIVA | shape + no NaNs |
| `test_scviva_normalized_expression_importance` | SCVIVA | shape + no NaNs |
| `test_scviva_neighbor_abundance` | SCVIVA | shape + no nulls |
| `test_scviva_differential_expression` | SCVIVA | returns DataFrame |
| `test_scviva_differential_abundance` | SCVIVA | writes to `adata.obsm["da_log_probs"]` |
| `test_resolvi_neighbor_abundance` | ResolVI | shape + no NaNs (semisupervised=True) |

## Key Decisions

- `get_neighbor_abundance` in `SpatialPredictiveMixin` uses Pyro's `sample_posterior` — default batch_size is now capped at `n_obs` to avoid Pyro plate subsample_size mismatch on small datasets.
- GIMVI and RNAStereoscope: `RNASeqMixin` additions attempted and reverted — module architectures incompatible with `RNASeqMixin.get_normalized_expression`. Spec caveat was correct.
- `differential_abundance` (from `VAEMixin`) returns `None` and stores in `adata.obsm["da_log_probs"]` — test assertions updated accordingly.
...
|------|--------------|
| 2026-04-12 (GIMVI + Stereoscope) | 76/76 |
| 2026-06-07 (SpatialPredictiveMixin + DE coverage) | 85/85 (non-regression) |
